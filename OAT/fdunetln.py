...
class FDUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.conv1(x) # (-1,C,H,W)
        x1 = self.db1(x1) # (-1.C*2,H,W)
        x2 = self.maxpool(x1) # (-1,C*2,H/2,W/2)
        x2 = self.db2(x2) # (-1,C*4,H/2,W/2)
        x3 = self.maxpool(x2) # (-1,C*4,H/4,W/4)
        x3 = self.db3(x3) # (-1,C*8,H/4,W/4)
        x4 = self.maxpool(x3) # (-1,C*8,H/8,W/8)
        x4 = self.db4(x4) # (-1,C*16,H/8,W/8)
        x5 = self.maxpool(x4) # (-1,C*16,H/16,W/16)
        x5 = self.db5(x5) # (-1,C*32,H/16,W/16)
        
        x6 = self.up1(x5,x4) # (-1,C*8,H/8,W/8)
        x6 = self.db6(x6) # (-1,C*16,H/8,W/8)
        x7 = self.up2(x6,x3) # (-1,C*4,H/4,W/4)
        x7 = self.db7(x7) # (-1,C*8,H/4,W/4)
        x8 = self.up3(x7,x2) # (-1,C*2,H/2,H/2)
        x8 = self.db8(x8) # (-1,C*4,H/2,W/2)
        x9 = self.up4(x8,x1) # (-1,C,H,W)
        x9 = self.db9(x9) # (-1,C*2,H,W)
        x10 = self.conv2(x9) # (-1,1,H,W)
